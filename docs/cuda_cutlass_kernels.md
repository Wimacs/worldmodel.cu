...
- runtime 接入保持 opt-in，不默认启用。360p、24 layer、4 step、`WORLD_VAE_FP16_NHWC=1`、`cache-window=8`、满 cache profile：默认 indexed warp path `attn=38.657ms/120`、`total=79.937ms`；half KV cache `attn=31.566ms/120`、`total=72.425ms`；CUTLASS materialized path `attn=7.713ms/120`、`total=48.453ms`。非 profile 长跑 18 个 chunk 后为 `total=41.877ms`、`chunk_fps=23.879`、`rgb_fps=95.517`，输出帧和 latent 统计稳定。
- 这版不是最终 attention：它 materialize `scores[H,T,Nkv]`，把 softmax 概率再 cast 到 half，并且为了让 AV 走 GEMM 复制/展开 GQA KV；runtime 里还为了拿 `Nkv` 做了一次 device-to-host 同步。scratch 默认限制为 2048 MiB，`WORLD_ATTN_D64_CUTLASS_MAX_SCRATCH_MIB` 可覆盖。下一版 attention 应该优先消掉 host sync，随后转向真正 fused online-softmax QK/AV，避免 scores/probs materialization 和 GQA KV 复制；不要回退到 cuBLAS。
