...
- attention v1.9 实现了 native block-sparse GQA FMHA，不调用不存在的“CUTLASS sparse attention API”，而是在保留 BSD-3-Clause 版权的 CUTLASS 3.5.1 FMHA mainloop 变体中加入 sparse block iterator。runtime 的逻辑 KV 顺序按 128-token block 表示；CUTLASS 每次处理 64-key tile 时，用 `physical_key_start = block_ids[logical_key / 128] * 128 + logical_key % 128` 直接定位 head-major FP16 ring cache。QK、FP32 online softmax 和 PV 仍由原 CUTLASS Tensor Core mainloop 完成，因此没有 materialize `scores/probs`，也不再 gather compact K/V。
- GQA 仍采用 v1.7 已验证的 row folding：`q[Hq,T,64] -> q[group*T,Hkv,64]`，FMHA 的 `num_heads=Hkv`，所以 K/V 不按 query head 重复。第一版有意保留 Q half bridge 和 output scatter，只消掉 K/V gather；这样可以单独验证 sparse iterator 的价值。runtime 通过 `WORLD_ATTN_D64_SPARSE_FMHA=1` opt-in，要求 `D=64`、`T % 128 == 0` 和 `Hq % Hkv == 0`。
- cache 收集没有增加 launch：现有 `collect_cache_frame_indices_kernel` 在生成 token indices 的同时写出物理 block ids。PyTorch extension 新增 batched `[B,Nblocks]` probe，覆盖不同 batch block 表、非连续物理块和 ring-wrap 顺序；完整 CUDA/PyTorch 测试 45 项通过。旧 compact FMHA bridge 和 sparse FMHA 在相同 block 展开下 half 输出逐元素完全相同。
- 真实 shape microbench `B=1,Hq=32,Hkv=16,Tq=128,Tk=1024,D=64`：旧 GQA FMHA bridge `0.0510ms`，native sparse `0.0459ms`，约快 `10.0%`。360p runtime scratch 从 `9.50 MiB` 降到 `1.00 MiB`；主模型 T=512 runtime scratch 从 `38.00 MiB` 降到 `4.00 MiB`。
- 完整 resident runtime A/B 都使用 24 layer、4 step、`cache-window=8`、11 个连续 chunk，覆盖 frozen denoise pass、cache write pass、满窗口和 ring wrap。360p 共 `180224` 个 latent float、主模型 T=512 共 `720896` 个 latent float，旧/新路径均逐字节一致。主模型满窗口 profile 的 120 次 attention 从 `24.881ms` 降到 `22.996ms`，约快 `7.6%`；非 profile transformer 最后 chunk 从 `117.147ms` 降到 `114.347ms`。这一版到此停止，不继续调 FMHA tile；下一版只有在 profile 仍指向 attention 时，才考虑让 QKV producer 直接写 folded half Q、让 out projection 直接消费 FMHA layout，以消掉剩余 Q/O bridge。
