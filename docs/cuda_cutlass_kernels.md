...
- 这仍不是最终 attention：它 materialize `scores[H,T,Nkv]`，把 softmax 概率再 cast 到 half，并且 grouped-M 还需要一次 output scatter。scratch 默认限制为 2048 MiB，`WORLD_ATTN_D64_CUTLASS_MAX_SCRATCH_MIB` 可覆盖。因为端到端收益只有约 0.6%，这版保持 opt-in，不默认开启；下一版 attention 应该转向真正 fused online-softmax QK/AV，避免 scores/probs materialization 和 scatter，而不是继续围绕 materialized 版本做无边界 tile 调参。
