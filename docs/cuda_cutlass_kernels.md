...
- 这仍不是最终 attention：它 materialize `scores[H,T,Nkv]`，把 softmax 概率再 cast 到 half，并且为了让 AV 走 GEMM 复制/展开 GQA KV。scratch 默认限制为 2048 MiB，`WORLD_ATTN_D64_CUTLASS_MAX_SCRATCH_MIB` 可覆盖。下一版 attention 应该转向真正 fused online-softmax QK/AV，避免 scores/probs materialization 和 GQA KV 复制；不要回退到 cuBLAS，也不要继续围绕 materialized 版本做无边界小调参。
