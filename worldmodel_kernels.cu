...
__global__ void silu_f32_to_f16_kernel(const float *x, cutlass::half_t *y, int64_t n) {
    int64_t i = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        y[i] = cutlass::half_t(wm_silu(x[i]));
    }
}

__global__ void cast_f32_to_f16_kernel(const float *x, cutlass::half_t *y, int64_t n) {
    int64_t i = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        y[i] = cutlass::half_t(x[i]);
    }
}

__global__ void gather_indexed_kv_d64_hq_f16_kernel(
...
__global__ void indexed_attention_d64_hkv_group_flash_f16_kv_kernel(
        const float *__restrict__ q,
        const __half *__restrict__ k,
        const __half *__restrict__ v,
        const int64_t *__restrict__ indices,
    ...
    __half *sh_k = smem_h;
    __half *sh_v = sh_k + WM_ATTN_D64_K_BLOCK * 64;
...
__global__ void taehv_add_bias_nhwc_f16_kernel(
        cutlass::half_t *out,
    ...
    int64_t i = (int64_t)blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= total) return;
    out[i] = cutlass::half_t(float(out[i]) + float(bias[i % C]));
}
...
torch::Tensor silu_to_half_cuda(torch::Tensor x) {
    ...
    int64_t n = x.numel();
    silu_f32_to_f16_kernel<<<div_up_i64(n, 256), 256, 0, at::cuda::getCurrentCUDAStream()>>>(
        x.data_ptr<float>(),
        reinterpret_cast<cutlass::half_t *>(y.data_ptr<at::Half>()),
        n);
    check_last_cuda_error("silu_f32_to_f16");
    return y;
}
...
torch::Tensor row_major_linear_fp16_tensorop_m64n64_cuda(torch::Tensor x, torch::Tensor w) {
    WM_CHECK_CUDA(x);
    WM_CHECK_CUDA(w);
    WM_CHECK_CONTIGUOUS(x);
    WM_CHECK_CONTIGUOUS(w);
    ...
    typename Gemm::Arguments args(
        ...
        {1.0f, 0.0f});
    Gemm gemm;
    check_cutlass_status(gemm.can_implement(args), "row_major_linear_fp16_tensorop_m64n64.can_implement");
    check_cutlass_status(gemm(args, nullptr, at::cuda::getCurrentCUDAStream()), "row_major_linear_fp16_tensorop_m64n64");
    check_last_cuda_error("row_major_linear_fp16_tensorop_m64n64");
    return y;
}
...
torch::Tensor row_major_linear_fp16_tensorop_splitk_cuda(torch::Tensor x, torch::Tensor w, int64_t split_k_slices_arg) {
    ...
    Gemm gemm;
    check_cutlass_status(gemm.can_implement(args), "row_major_linear_fp16_tensorop_splitk.can_implement");
    check_cutlass_status(gemm(args, workspace_ptr, at::cuda::getCurrentCUDAStream()), "row_major_linear_fp16_tensorop_splitk");
    check_last_cuda_error("row_major_linear_fp16_tensorop_splitk");
    return y;
}

torch::Tensor row_major_linear_fp16_input_tensorop_splitk_cuda(torch::Tensor x, torch::Tensor w, int64_t split_k_slices_arg) {
    WM_CHECK_CUDA(x);
    WM_CHECK_CUDA(w);
    WM_CHECK_CONTIGUOUS(x);
    WM_CHECK_CONTIGUOUS(w);
    ...
    void *workspace_ptr = workspace_size ? workspace.data_ptr<uint8_t>() : nullptr;
    Gemm gemm;
    check_cutlass_status(gemm.can_implement(args), "row_major_linear_fp16_input_tensorop_splitk.can_implement");
    check_cutlass_status(gemm(args, workspace_ptr, at::cuda::getCurrentCUDAStream()), "row_major_linear_fp16_input_tensorop_splitk");
    check_last_cuda_error("row_major_linear_fp16_input_tensorop_splitk");
    return y;
}
...
torch::Tensor indexed_attention_half_kv_flash_cuda(
    ...
    TORCH_CHECK(k.sizes() == v.sizes(), "k and v shapes must match");
    TORCH_CHECK(q.size(0) == k.size(0), "B mismatch");
    TORCH_CHECK(q.size(3) == 64 && k.size(3) == 64, "indexed_attention_half_kv_flash currently supports D=64");
    ...
    TORCH_CHECK(Hq % Hkv == 0, "Hq must be divisible by Hkv for GQA");
    int group = Hq / Hkv;
    TORCH_CHECK(group > 0 && group <= WM_ATTN_D64_FLASH_WARPS, "unsupported GQA group for half-KV flash prototype");

    auto out = torch::empty_like(q);
    int q_per_h = WM_ATTN_D64_FLASH_WARPS / group;
    if (q_per_h < 1) q_per_h = 1;
    int q_blocks = div_up_i64(Tq, q_per_h);
    size_t smem = (size_t)2 * WM_ATTN_D64_K_BLOCK * 64 * sizeof(__half);
    indexed_attention_d64_hkv_group_flash_f16_kv_kernel<<<B * Hkv * q_blocks, 32 * WM_ATTN_D64_FLASH_WARPS, smem, at::cuda::getCurrentCUDAStream()>>>(
        q.data_ptr<float>(),
        reinterpret_cast<const __half *>(k.data_ptr<at::Half>()),
        reinterpret_cast<const __half *>(v.data_ptr<at::Half>()),
        indices.data_ptr<int64_t>(),
        ...
        Tk,
        (float)scale);
    check_last_cuda_error("indexed_attention_d64_hkv_group_flash_f16_kv");
    return out;
}
...
torch::Tensor indexed_attention_half_kv_cutlass_cuda(
    ...
    TORCH_CHECK(k.sizes() == v.sizes(), "k and v shapes must match");
    TORCH_CHECK(q.size(0) == k.size(0), "B mismatch");
    TORCH_CHECK(q.size(3) == 64 && k.size(3) == 64, "indexed_attention_half_kv_cutlass currently supports D=64");
    ...
    auto k_compact = torch::empty({B, Hq, Nkv, 64}, half_opts);
    auto v_compact = torch::empty({B, Hq, Nkv, 64}, half_opts);
    auto scores = torch::empty({B, Hq, Tq, Nkv}, q.options());
    auto probs_h = torch::empty({B, Hq, Tq, Nkv}, half_opts);
    auto out = torch::empty_like(q);
    ...
    cast_f32_to_f16_kernel<<<div_up_i64(q_elems, 256), 256, 0, at::cuda::getCurrentCUDAStream()>>>(
        ...
        q_elems);
    check_last_cuda_error("attention_cutlass_q_cast");

    int64_t kv_compact_elems = (int64_t)B * Hq * Nkv * 64;
    gather_indexed_kv_d64_hq_f16_kernel<<<div_up_i64(kv_compact_elems, 256), 256, 0, at::cuda::getCurrentCUDAStream()>>>(
        reinterpret_cast<const __half *>(k.data_ptr<at::Half>()),
        reinterpret_cast<const __half *>(v.data_ptr<at::Half>()),
        indices.data_ptr<int64_t>(),
        ...
        Nkv,
        Tk);
    check_last_cuda_error("attention_cutlass_gather_kv");

    int batch_count = B * Hq;
    ...
    typename GemmQK::Arguments qk_args(
        ...
        batch_count);
    GemmQK qk_gemm;
    check_cutlass_status(qk_gemm.can_implement(qk_args), "indexed_attention_half_kv_cutlass.qk.can_implement");
    check_cutlass_status(qk_gemm(qk_args, nullptr, at::cuda::getCurrentCUDAStream()), "indexed_attention_half_kv_cutlass.qk");
    check_last_cuda_error("indexed_attention_half_kv_cutlass_qk");

    softmax_rows_inplace_f32_kernel<<<B * Hq * Tq, 256, 0, at::cuda::getCurrentCUDAStream()>>>(
        scores_ptr,
        B * Hq * Tq,
        Nkv);
    check_last_cuda_error("indexed_attention_half_kv_cutlass_softmax");

    int64_t probs_elems = (int64_t)B * Hq * Tq * Nkv;
    cast_f32_to_f16_kernel<<<div_up_i64(probs_elems, 256), 256, 0, at::cuda::getCurrentCUDAStream()>>>(
        ...
        probs_elems);
    check_last_cuda_error("attention_cutlass_probs_cast");
    ...
    typename GemmAV::Arguments av_args(
        ...
        batch_count);
    GemmAV av_gemm;
    check_cutlass_status(av_gemm.can_implement(av_args), "indexed_attention_half_kv_cutlass.av.can_implement");
    check_cutlass_status(av_gemm(av_args, nullptr, at::cuda::getCurrentCUDAStream()), "indexed_attention_half_kv_cutlass.av");
    check_last_cuda_error("indexed_attention_half_kv_cutlass_av");
    return out;
}
...
torch::Tensor taehv_conv3x3_cutlass_implicit_nhwc_fp16_cuda(torch::Tensor x, torch::Tensor weight, torch::Tensor bias) {
    WM_CHECK_CUDA(x);
    WM_CHECK_CUDA(weight);
    WM_CHECK_CUDA(bias);
    WM_CHECK_CONTIGUOUS(x);
    ...
    Element *x_ptr = const_cast<Element *>(reinterpret_cast<const Element *>(x.data_ptr<at::Half>()));
    Element *weight_ptr = const_cast<Element *>(reinterpret_cast<const Element *>(weight.data_ptr<at::Half>()));
    Element *out_ptr = reinterpret_cast<Element *>(out.data_ptr<at::Half>());
    typename ImplicitGemm::Arguments args(
        problem,
        cutlass::TensorRef<Element, Layout>(x_ptr, Layout::packed(input_size)),
    ...
    ImplicitGemm implicit_gemm;
    check_cutlass_status(implicit_gemm.can_implement(args), "taehv_conv3x3_cutlass_implicit_nhwc_fp16.can_implement");
    size_t workspace_size = implicit_gemm.get_workspace_size(args);
    auto workspace = torch::empty({(int64_t)workspace_size}, x.options().dtype(torch::kUInt8));
    void *workspace_ptr = workspace_size ? workspace.data_ptr<uint8_t>() : nullptr;
    cudaStream_t stream = at::cuda::getCurrentCUDAStream();
    check_cutlass_status(implicit_gemm(args, workspace_ptr, stream), "taehv_conv3x3_cutlass_implicit_nhwc_fp16");
    check_last_cuda_error("taehv_conv3x3_cutlass_implicit_nhwc_fp16");

    taehv_add_bias_nhwc_f16_kernel<<<div_up_i64((int64_t)N * H * W * C_out, 256), 256, 0, stream>>>(
        out_ptr,
        reinterpret_cast<const Element *>(bias.data_ptr<at::Half>()),
        (int64_t)N * H * W * C_out,
        C_out);
    check_last_cuda_error("taehv_conv3x3_cutlass_implicit_nhwc_fp16_bias");
    return out;
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("silu_to_half", &silu_to_half_cuda, "WorldModel SiLU fused with FP16 cast (CUDA)");
    ...
    m.def("taehv_conv3x3_cutlass_implicit_nhwc_fp16", &taehv_conv3x3_cutlass_implicit_nhwc_fp16_cuda, "TAEHV 3x3 Conv2d using CUTLASS implicit-GEMM NHWC/KRSC probe (CUDA, fp16)");
